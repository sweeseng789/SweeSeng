...
//	resendTree.Clear(__FILE__, __LINE__);
	...
	for ( i = 0; i < NUMBER_OF_PRIORITIES; i++ )
	{
		...
		{
			if (( sendPacketSet[ i ] ) [ j ]->data)
				rakFree_Ex(( sendPacketSet[ i ] ) [ j ]->data, __FILE__, __LINE__ );
			ReleaseToInternalPacketPool( ( sendPacketSet[ i ] ) [ j ] );
		}

		sendPacketSet[ i ].ClearAndForceAllocation( 32, __FILE__,__LINE__ ); // Preallocate the send lists so we don't do a bunch of reallocations unnecessarily
	}
	...
	*/
	...
	for (int i=0; i < DATAGRAM_MESSAGE_ID_ARRAY_LENGTH; i++)
		datagramMessageIDArray[i].messages.Clear(false, __FILE__, __LINE__);
...
//	int headerLength = BITS_TO_BYTES( GetBitStreamHeaderLength( internalPacket, true ) );
	...
	sendPacketSet[ internalPacket->priority ].Push( internalPacket, __FILE__, __LINE__  );
...

{
	...
	while (delayList.Size())
	{
		if (delayList.Peek()->sendTime <= timeMs)
		{
			...
			SocketLayer::Instance()->SendTo( dat->s, dat->data, dat->length, dat->systemAddress.binaryAddress, dat->systemAddress.port, dat->remotePortRakNetWasStartedOn_PS3 );
			...
			if (unreliableLinkedListHead)
			{
				...
				while (1)
				{
					if (time > cur->creationTime+(CCTimeType)unreliableTimeout)
					{
						...
						rakFree_Ex(cur->data, __FILE__, __LINE__ );
						cur->data=0;
...
//			idList->messages.Clear(__FILE__, __LINE__);
			...
			datagramMessageIDList.messages.Clear(true, __FILE__,__LINE__);
...
void ReliabilityLayer::SendBitStream( SOCKET s, SystemAddress systemAddress, RakNet::BitStream *bitStream, RakNetRandom *rnr, unsigned short remotePortRakNetWasStartedOn_PS3)
{
	...
	if (minExtraPing > 0 || extraPingVariance > 0)
	{
		...
		if (delay > 0)
		{
			...
			dat->systemAddress=systemAddress;
...
void ReliabilityLayer::ResetPacketsAndDatagrams(void)
{
	...
	packetsToDeallocThisUpdate.Clear(false, __FILE__, __LINE__);
	packetsToSendThisUpdateDatagramBoundaries.Clear(false, __FILE__, __LINE__);
	datagramsToSendThisUpdateIsPair.Clear(false, __FILE__, __LINE__);
	datagramSizesInBytes.Clear(false, __FILE__, __LINE__);
	datagramSizeSoFar=0;
}
//-------------------------------------------------------------------------------------------------------
