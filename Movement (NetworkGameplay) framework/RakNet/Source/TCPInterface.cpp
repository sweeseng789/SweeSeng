...
bool TCPInterface::SendList( const char **data, const unsigned int *lengths, const int numParameters, SystemAddress systemAddress, bool broadcast )
{
	...
	{
		// Send to this player
		if (systemAddress.systemIndex>=0 &&
			systemAddress.systemIndex<remoteClientsLength &&
...
void TCPInterface::CloseConnection( SystemAddress systemAddress )
{
	...
	if (systemAddress.systemIndex>=0 && systemAddress.systemIndex<remoteClientsLength && remoteClients[systemAddress.systemIndex].systemAddress==systemAddress)
	{
