...
void CCRakNetUDT::UpdateNextAllowedSend(CCTimeType curTime, uint32_t numBytes)
{
	...
	else
	{
		...
		nextAllowedSend=(CCTimeType)(curTime+(double)numBytes*SND);
	}
}
// ****************************************************** UNIT TESTING ******************************************************
...
CCTimeType CCRakNetUDT::GetSenderRTOForACK(void) const
{
	...
	return (CCTimeType)(RTT + 4.0 * RTTVar + SYN);
}
// ----------------------------------------------------------------------------------------------------------------------------
CCTimeType CCRakNetUDT::GetRTOForRetransmission(void) const
{
	...
	CCTimeType time = (CCTimeType) ((ExpCount * (RTT + 4.0 * RTTVar) + (double) SYN));
...
{
#if CC_TIME_TYPE_BYTES==4
	const BytesPerMicrosecond factor = 1.0 / (BytesPerMicrosecond) MAXIMUM_DATAGRAM_SIZE;
#else
